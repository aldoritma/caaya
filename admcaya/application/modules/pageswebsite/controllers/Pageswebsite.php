...
class Pageswebsite extends Authorize_Controller {
	
	function __construct()
	{
		parent::__construct();
		$this->data['current_page'] = 'dashboard';	
		$this->load->model(array('pageswebsite_m','global_m'));
	...
	public function index()
	{
		...
		$this->data['totaldata'] = $this->pageswebsite_m->total_data();
        $offset = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
        $config['base_url'] = base_url('about/index/page/');
        $config['total_rows'] = $this->pageswebsite_m->total_data();
        $config['per_page'] = 10;
        ...
        $this->data['listdata'] = $this->pageswebsite_m->allabout($config['per_page'], $offset);
			...
			if ($this->form_validation->run($this))
			{
				if (isset($_FILES['userfile']))
				{
					if ($this->upload->do_upload('userfile')) 
					{
						...
						if ($this->pageswebsite_m->create($this->input->post(NULL, TRUE), $data['file_name']))
						{
				...
				else
				{
					...
					if ($this->pageswebsite_m->create($this->input->post(NULL, TRUE)))
					{	
						// Flash message
	...
	function edit($id = false)
	{
		...
		if ( ! $this->data['dataslide'] = $this->pageswebsite_m->get_one($id)) show_404();
	...
	function editdata($id = false)
	{
		...
		if (is_ajax)
		{
			$this->data['image'] = $this->pageswebsite_m->get_image($id);
			// title
			...
			if ($this->form_validation->run($this))
			{
				if (isset($_FILES['userfile']))
				{
					if ($this->upload->do_upload('userfile')) 
					{
						...
						/* Insert data Content with image into table */
						if ($this->pageswebsite_m->update($id,$this->input->post(NULL, TRUE), $data['file_name']))
						{
				...
				else
				{
					...
					if ($this->pageswebsite_m->update($id,$this->input->post(NULL, TRUE)))
					{	
	...
	function delete($id = false){
		...
		if (is_ajax)
		{
			$this->data['content'] = $this->pageswebsite_m->get_one($id);
			if ($this->pageswebsite_m->delabout($id))
			{

				@unlink("../assets/about/original_".$this->data['content']['img']);
				@unlink("../assets/about/417x417_".$this->data['content']['img']);
				$this->pageswebsite_m->delslideall($this->data['content']['id']);
	...
	{
		if ( ! $this->data['dataslide'] = $this->pageswebsite_m->get_about($id)) show_404();
		$this->data['totaldata'] = $this->pageswebsite_m->total_dataslide($id);
        $offset = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
        ...
        $this->pagination->initialize($config);
        $this->data['paging'] = $this->pagination->create_links();
        $this->data['slidedata'] = $this->pageswebsite_m->allslide($id,$config['per_page'], $offset);
        $this->data['number'] = $offset + ($offset - 1) * ($config['per_page'] - 1);
	...
	function delslide($id = false)
	{
		if (is_ajax)
		{
			...
			if ($this->pageswebsite_m->delslide($id))
			{
	...
	function editSaveslide($idcat = false)
	{
		if (is_ajax)
		{
			...
			if ($this->form_validation->run($this))
			{
				if ($this->pageswebsite_m->updateSlide($idcat, $this->input->post(NULL, TRUE)))
				{
	...
	{
		if ( ! $this->data['dataslide'] = $this->pageswebsite_m->get_aboutid($id)) show_404();
	...
	function saveslide($id = false)
	{
		...
		if (is_ajax)
		{
			...
			if ($this->form_validation->run($this))
			{
				if ($this->pageswebsite_m->saveslide($id,$this->input->post(NULL, TRUE)))
				{
