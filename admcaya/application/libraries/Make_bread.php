...
class Make_bread
{
    ...
    public function __construct()
    {
        ...
        {
            $this->_breadcrumb[] = array('title'=>$this->_include_home, 'href'=>rtrim(base_url('dashboard'),'/'),'attr'=>'');
        }
    }

    public function add($title = NULL, $href = '', $segment = FALSE, $active = FALSE)
    {
        ...
        $this->_breadcrumb[] = array('title' => $title, 'href' => $href, 'attr'=>$active);
    }
    ...
    public function output()
    {
        ...
        if(sizeof($this->_breadcrumb) > 0)
        {
            foreach($this->_breadcrumb as $key=>$crumb)
            {
                ...
                {
                    $output .= anchor($crumb['href'],$crumb['title'], $crumb['attr']);
                }
                else
                {
                    $output .= $crumb['title'];
